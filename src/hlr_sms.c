...
/***********************************************************************
 * SEND-ROUTING-INFO-FOR-SM handling
 ***********************************************************************/

void rx_send_routing_info_for_sm_req(struct osmo_gsup_req *req)
{
    struct hlr_subscriber subscr;
    /*
     * We initialize response to 0.
     * osmo_gsup_req_respond() will automatically derive the correct message type:
     * - If error=false: Result type (Request Type + 2, binary ...10)
     * - If error=true:  Error type  (Request Type + 1, binary ...01)
     */
    struct osmo_gsup_message response = {0};
    int rc;

    /* 1. Check if MSISDN is present (Mandatory IE in SRI-for-SM) */
    if (!req->gsup.msisdn_enc || !req->gsup.msisdn_enc_len) {
        osmo_gsup_req_respond_err(req, GMM_CAUSE_INV_MAND_INFO,
                      "missing MSISDN");
        return;
    }

    /* 2. Decode MSISDN (BCD -> String) */
    char msisdn[GSM48_MI_SIZE + 1];
    rc = gsm48_decode_bcd_number2(msisdn, sizeof(msisdn),
                      req->gsup.msisdn_enc,
                      req->gsup.msisdn_enc_len, 0);
    if (rc < 0) {
        osmo_gsup_req_respond_err(req, GMM_CAUSE_INV_MAND_INFO,
                      "invalid MSISDN encoding");
        return;
    }

    /* 3. Query Database */
    rc = db_subscr_get_by_msisdn(g_hlr->dbc, msisdn, &subscr);
    if (rc < 0) {
        osmo_gsup_req_respond_err(req, GMM_CAUSE_IMSI_UNKNOWN,
                      "MSISDN unknown");
        return;
    ...
    /* 4. Populate IMSI in response */
    OSMO_STRLCPY_ARRAY(response.imsi, subscr.imsi);
    response.msisdn_enc = req->gsup.msisdn_enc;
    response.msisdn_enc_len = req->gsup.msisdn_enc_len;

    /* 5. Populate Location Info (MSC/SGSN Number) */
    /* Priority: MSC (CS domain) > SGSN (PS domain) */
    if (subscr.vlr_number[0]) {
        /* CS domain attached, return MSC number */
        /* Note: We use hlr_enc field to carry the Network Node Number */
        int8_t vlr_num_buf[32];
        int len = gsm48_encode_bcd_number(vlr_num_buf, sizeof(vlr_num_buf), 0, subscr.vlr_number);
        if (len >1){
            response.hlr_enc = vlr_num_buf + 1;
            response.hlr_enc_len = vlr_num_buf[0];
        }else{
            response.hlr_enc = (uint8_t *)subscr.vlr_number;
            response.hlr_enc_len = strlen(subscr.vlr_number) + 1;
        }
    } else if (subscr.sgsn_number[0]) {
        /* PS domain attached, return SGSN number */
        response.hlr_enc = (uint8_t *)subscr.sgsn_number;
        response.hlr_enc_len = strlen(subscr.sgsn_number) + 1;
    } else {
        /* Subscriber not attached */
        osmo_gsup_req_respond_err(req, GMM_CAUSE_IMPL_DETACHED,
                      "Subscriber not attached (no VLR/SGSN info)");
        return;
    }

    /* 6. Send Success Response */
    /* This will generate OSMO_GSUP_MSGT_SEND_ROUTING_INFO_RESULT (0x0E if Req is 0x0C) */
    osmo_gsup_req_respond(req, &response, false, true);
}
